...
This is a small standalone Hibernate harness for two questions:

1. After `detach()` or `clear()`, do dirty-check snapshots or entity values remain reachable long enough to be recovered?
2. Can timing differences reveal persistence-context membership?
...
- Persists a single entity with a large `byte[]` payload.
- Loads that entity into a session, mutates it without flushing, and then applies either `detach()` or `clear()`.
